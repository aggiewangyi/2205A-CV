    ...
    with open(args.output, "w") as fout:
        with open(args.image_feats, "r") as fin:
            for line in tqdm(fin):
                ...
                image_feat_tensor = torch.tensor([image_feat], dtype=torch.float) # [1, feature_dim]
                idx = 0
                while idx < len(text_ids):
                    ...
                    text_feats_tensor = torch.from_numpy(text_feats_array[idx : min(idx + args.eval_batch_size, len(text_ids))]) # [batch_size, feature_dim]
