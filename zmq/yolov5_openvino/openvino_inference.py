...
class OpenVino:
    def __init__(self, model_path, confidence_thres=0.25, iou_thres=0.5):
        ...
        self.classes = self.yaml_load('yolov5s_openvino_model/yolov5s.yaml')['names']
    ...
    def preprocess(self, input_image):
        ...
        # spend time =(time.time()- start time)*1000
        # print("norm time:",spend_time)
        # image data = np.ascontiquousarray(image_data)
        return image_data
    ...
    def yaml_load(self, file="data.yaml", append_filename=False):
        ...
        with open(file, errors="ignore", encoding="utf-8") as f:
            ...
            # always return a dict (yaml.safe load() may return None for empty files)
            if append_filename:
